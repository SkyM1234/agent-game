...
def _api_config() -> Dict[str, str]:
    try:
        ...
        _die(f"Reading .env requires python-dotenv. {_dependency_hint('python-dotenv')}")
...
def _ensure_api_key(dry_run: bool) -> None:
    if _api_config()["api_key"]:
        print("OPENAI_API_KEY is configured in project .env.", file=sys.stderr)
        return
    if dry_run:
        _warn("OPENAI_API_KEY is not set; dry-run only.")
        return
    _die("OPENAI_API_KEY is not set. Add it to the project root .env file.")
...
def _create_async_client():
    ...
    return AsyncOpenAI(**_api_config())
